...
def MainToolbar(root, parentpanel = None, use_wrapsizer = False):
    ...
    root.serialport.SetToolTip(wx.ToolTip(_("Select Port Ventilator is connected to")))
    root.rescanports()
    ...
    root.baud.SetToolTip(wx.ToolTip(_("Select Baud rate for Ventilator communication")))
    try:
    ...
    if not hasattr(root, "connectbtn"):
        root.connectbtn = make_autosize_button(parentpanel, _("Connect"), root.connect, _("Connect to the Ventilator"))
    ...
    if not hasattr(root, "resetbtn"):
        root.resetbtn = make_autosize_button(parentpanel, _("Reset"), root.reset, _("Reset the Ventilator"))
