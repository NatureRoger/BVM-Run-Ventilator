...
class pronsole(cmd.Cmd):
    ...
    def preloop(self):
        self.log(_("Welcome to the BVM_Run console! Type \"help\" for a list of available commands."))
    ...
    def help_gcodes(self):
        self.log("Gcodes are passed through to the BVM_Run as they are")
    ...
    def default(self, l):
        if l[0].upper() in self.commandprefixes.upper():
            if self.p and self.p.online:
                ...
                self.logError(_("BVM_Run is not online."))
            return
        elif l[0] == "@":
            if self.p and self.p.online:
                if not self.p.loud:
    ...
    def do_exit(self, l):
        ...
        self.log("Disconnecting from BVM_Run...")
...
(this will terminate the print)."))
    ...
    def help_exit(self):
        self.log(_("Disconnects from the BVM_Run and exits the program."))
    ...
    def online(self):
        self.log("\rBVM_Run is now online")
        self.write_prompt()
    ...
    def help_disconnect(self):
        self.log("Disconnects from the BVM_Run")
    ...
    def help_block_until_online(self, l):
        self.log("Blocks until BVM_Run is online")
    ...
    def do_upload(self, l):
        ...
        if not self.p.online:
            self.logError(_("Not connected to BVM_Run."))
            return
        self._do_load(filename)
    ...
    def help_print(self):
        if not self.fgcode:
            self.log(_("Send a loaded gcode file to the BVM_Run. Load a file with the load command first."))
        else:
            self.log(_("Send a loaded gcode file to the BVM_Run. You have %s loaded right now.") % self.filename)
    ...
    def do_ls(self, l):
        if not self.p.online:
            self.logError(_("BVM_Run is not online. Please connect to it first."))
            return
        self._do_ls(True)
    ...
    def help_reset(self):
        self.log(_("Resets the BVM_Run."))

    def do_sdprint(self, l):
        if not self.p.online:
            self.log(_("BVM_Run is not online. Please connect to it first."))
            return
        self._do_ls(False)
    ...
    def do_eta(self, l):
        if not self.p.printing:
            self.logError(_("BVM_Run is not currently printing. No ETA available."))
        else:
    ...
    def do_monitor(self, l):
        ...
        if not (self.p.printing or self.sdprinting):
            self.logError(_("BVM_Run is not printing. Please print something before monitoring."))
            return
        self.log(_("Monitoring BVM_Run, use ^C to interrupt."))
        if len(l):
            try:
                interval = float(l)
            except:
    ...
    def do_move(self, l):
        if len(l.split()) < 2:
            ...
            return
        if self.p.printing:
            self.logError(_("BVM_Run is currently printing. Please pause the print before you issue manual commands."))
            return
        if not self.p.online:
            self.logError(_("BVM_Run is not online. Unable to move."))
            return
        l = l.split()
        if l[0].lower() == "x":
    ...
    def do_extrude(self, l, override = None, overridefeed = 300):
        ...
        if not self.p.online:
            self.logError("BVM_Run is not online. Unable to extrude.")
            return
        if self.p.printing:
            self.logError(_("BVM_Run is currently printing. Please pause the print before you issue manual commands."))
            return
    ...
    def do_reverse(self, l):
        ...
        feed = self.settings.e_feedrate  # default speed
        if not self.p.online:
            self.logError(_("BVM_Run is not online. Unable to reverse."))
            return
        if self.p.printing:
            self.logError(_("BVM_Run is currently printing. Please pause the print before you issue manual commands."))
            return
